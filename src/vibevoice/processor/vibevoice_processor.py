...
_DEFAULT_TOKENIZER_PATH = str(_REPO_ROOT / "model" / "Qwen2.5-1.5B-tokenizer")
...
class VibeVoiceProcessor:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        # Load tokenizer - try from model path first, then fallback to local
        language_model_pretrained_name = config.get("language_model_pretrained_name", None) or kwargs.pop("language_model_pretrained_name", _DEFAULT_TOKENIZER_PATH)
        candidate = Path(language_model_pretrained_name)
        if not candidate.is_absolute():
            resolved = _REPO_ROOT / candidate
            if resolved.exists():
                language_model_pretrained_name = str(resolved)
            elif Path(_DEFAULT_TOKENIZER_PATH).exists():
                language_model_pretrained_name = _DEFAULT_TOKENIZER_PATH
